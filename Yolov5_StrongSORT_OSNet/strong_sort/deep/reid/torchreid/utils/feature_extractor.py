...
class FeatureExtractor(object):
    ...
    def __init__(
        ...
        model = models.build_model(
            model_name,
            num_classes=1,
            pretrained=not (model_path and utils.check_isfile(model_path)),
            use_gpu=device.startswith('cuda')
        )
        model.eval()

        if verbose:
            num_params, flops = utils.compute_model_complexity(
        ...
        if model_path and utils.check_isfile(model_path):
            utils.load_pretrained_weights(model, model_path)
