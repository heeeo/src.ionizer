...
class App:
    def __init__(self, video_source):
        ...
        self.cam_view.grid(row=0, column=0, padx=20, pady=20, sticky='nsew')
    ...
    def update(self, det_data):
        ...
        for cls in cls_set:
            ...
            if cls == 0: # ionizer
                for ion_det in cls_det_list:
                    ...
                    # 공정중 det_area_id set update
                        ...
                        self.insert_log(self.currState(det_area_ids))
                    
                    # 공정종료 이벤트
                    if False and (self.curr_process['stat'] == 1) and (self.det_area_x_max < xc):  # or xc < self.det_area_x_min
                        det_area_ids = self.curr_process['det_area_ids']
            ...
            elif cls == 3 or cls == 4: # 3: lan_port, 4: volt_check_new
    ...
    def currState(self, det_area_ids) -> str:
        s  = f"det_area_ids : {det_area_ids}, led_cnt: {self.curr_process['led_chk_cnt']}, volt_cnt: {self.curr_process['volt_chk_cnt']}"
        s += f", led_check: {self.curr_process['led_check']}, volt_check: {self.curr_process['volt_check']}"
        return s
