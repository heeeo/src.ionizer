...
class App:
    ...
    def get_nearest_ionizer(self, port_lan: DetectObject) -> Ionizer:
    ...
    def update(self, frame_idx, bboxes, id, cls):
        ...
        if cls == 1: # port_lan
            ...
            ionizer: Ionizer = self.get_nearest_ionizer(port_lan)
        ...
        if cls == 4: # port
            port = DetectObject(frame_idx, bboxes, id, cls)
            ionizer: Ionizer = self.get_nearest_ionizer(port)
            if ionizer is not None:
                if id in ionizer.ports:
                    ionizer.ports[id].update(frame_idx, bboxes)
                else:
                    ionizer.ports[id] = port    # 해당 ionizer 객체에 port_lan 등록
                    self.insert_log(f"port({id}) added to ionizer({ionizer.id}), total current ports: {len(ionizer.ports)}")
                    # volt_check_needed 수정
                    if ionizer.volt_check_needed == False and len(ionizer.ports) >= 3:
                        ionizer.volt_check_needed = True
                        self.insert_log(f"ionizer volt check type changed to 'needed' : ionizer(id={ionizer.id})")
        return
    ...
    def update_port_set(self, port_set):
        all_ionizers = self.ionizers["all"]
        ionizer :Ionizer
        for ion_id, ionizer in all_ionizers.items():
            ports = ionizer.ports
            if len(ports) > 0:
                ...
                for p_id, port in ports.items():
                    if p_id not in port_set:
                        ids_to_remove.append(p_id)
                for p_id in ids_to_remove:
                    ports.pop(p_id, None)
                    self.insert_log(f"port removed : [{p_id}] in ionizer({ion_id})")
                    # ionizer.show_portlans(self)

    def update_after_frame(self, ionizer_set, portlan_set, port_set):
        self.update_ionizer_set(ionizer_set)
        self.update_portlan_set(portlan_set)
        self.update_port_set(port_set)
